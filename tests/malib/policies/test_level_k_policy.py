...
class LevelKPolicyTest(tf.test.TestCase):
    ...
    def test_actions_and_log_pis_symbolic(self):
        ...
        # all_actions = self.level_k_policy.get_actions_np(observations_tf, 3)
        # all_log_pis = self.level_k_policy.log_pis_np([observations_tf], 3, all_actions)
        # print(all_actions)
        # print(all_log_pis)

        # self.assertEqual(all_actions[0].shape, (2, *self.env.action_space.shape))
        # self.assertEqual(all_log_pis[0].shape, (2, 1))
