...
class RelaxedSoftmaxPolicy(StochasticPolicy):
    def __init__(
        ...
        **kwargs
        ...
        def action_fn(inputs):
            ...
            return tfp.distributions.RelaxedOneHotCategorical(
        ...
        deterministic_actions = tf.keras.layers.Lambda(
            lambda logits: tf.math.softmax(logits)
        )(logits)
        ...
        def log_pis_fn(inputs):
            ...
            actions_dist = tfp.distributions.RelaxedOneHotCategorical(
                temperature, logits=logits
            )
        ...
        log_pis = tf.keras.layers.Lambda(log_pis_fn)(
            [self._temperature, logits, actions]
        )

        log_pis_for_action_input = tf.keras.layers.Lambda(log_pis_fn)(
            [self._temperature, logits, self.actions_input]
        )
        ...
        self.diagnostics_model = tf.keras.Model(
            self.condition_inputs, (logits, log_pis, actions)
        )
    ...
    def get_diagnostics(self, conditions):
        ...
        covariances.
        """
        (logits_np, log_pis_np, actions_np) = self.diagnostics_model.predict(conditions)

        return OrderedDict(
            {
                "{}/logits-mean".format(self._name): np.mean(logits_np),
