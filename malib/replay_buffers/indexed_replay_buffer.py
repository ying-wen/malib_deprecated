...
class IndexedReplayBuffer(Serializable):
    def __init__(
        ...
        self._terminal_dim = terminal_dim
        ...
        if self._reward_dim is None:
            self._rewards = np.zeros(self._max_buffer_size)
        else:
            self._rewards = np.zeros((self._max_buffer_size, self._reward_dim))
        if self._terminal_dim is None:
            self._terminals = np.zeros(self._max_buffer_size, dtype="uint8")
        else:
            self._terminals = np.zeros(
                (self._max_buffer_size, self._terminal_dim), dtype="uint8"
            )

        if self._opponent_action_dim is not None:
            self._opponent_actions = np.zeros(
                (self._max_buffer_size, self._opponent_action_dim)
            )
    ...
    def batch_by_indices(self, indices):
        ...
        if self._opponent_action_dim is not None:
            batch["opponent_actions"] = self._opponent_actions[indices].astype(
                np.float32
            )
        return batch
    ...
    def __setstate__(self, d):
        ...
        self._size = d["size"]
        if self._opponent_action_dim is not None:
            self._opponent_actions = np.fromstring(d["o_a"]).reshape(
