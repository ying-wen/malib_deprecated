...
class IndexedRolloutReplayBuffer(Serializable):
    def __init__(
        ...
        self._observations = np.zeros(
            (self._max_buffer_size, self._max_rollout_length, self._observation_dim),
            dtype=np.float32,
        )
        ...
        self._actions = np.zeros(
            (self._max_buffer_size, self._max_rollout_length, self._action_dim),
            dtype=np.float32,
        )
        self._rewards = np.zeros(
            (self._max_buffer_size, self._max_rollout_length), dtype=np.float32
        )
        self._terminals = np.zeros(
            (self._max_buffer_size, self._max_rollout_length), dtype="uint8"
        )
        if self._opponent_action_dim is not None:
            self._opponent_actions = np.zeros(
                (
                    self._max_buffer_size,
                    self._max_rollout_length,
        ...
        # current episode
        self._cur_observations = np.zeros(
            (self._max_rollout_length, self._observation_dim), dtype=np.float32
        )
        self._cur_next_obs = np.zeros(
            (self._max_rollout_length, self._observation_dim), dtype=np.float32
        )
        self._cur_actions = np.zeros(
            (self._max_rollout_length, self._action_dim), dtype=np.float32
        )
        ...
        if self._opponent_action_dim is not None:
            self._cur_opponent_actions = np.zeros(
                (self._max_rollout_length, self._opponent_action_dim), dtype=np.float32
            )
    ...
    def _cur_advance(self):
        ...
        if (
            self._cur_terminals[self._cur_top] == 1
            or self._cur_top == self._max_rollout_length - 1
        ):
            self._add_rollout(
                observations=self._cur_observations[: self._cur_top, ...],
    ...
    def batch_rollout_by_indices(self, indices):
        batch = dict(
            observations_rollout=self._observations[indices],
            actions_rollout=self._actions[indices],
            rewards_rollout=self._rewards[indices],
            terminals_rollout=self._terminals[indices],
