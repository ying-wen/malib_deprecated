...
def add_target_actions(batch_n, agents, batch_size, multiagent=True):
    target_actions_n = []
    for i, agent in enumerate(agents):
        target_actions_n.append(
            agent.act(batch_n[i]["next_observations"], use_target=True)
        )
    if multiagent:
        for i in range(len(agents)):
            target_actions = target_actions_n[i]
            opponent_target_actions = np.reshape(
                np.delete(deepcopy(target_actions_n), i, 0), (batch_size, -1)
            )
            target_actions = np.concatenate(
                (target_actions, opponent_target_actions), 1
            )
            # assert target_actions.shape[0] == batch_size
            batch_n[i]["target_actions"] = target_actions
    else:
        batch_n[0]["target_actions"] = target_actions_n[0]
    return batch_n
...
def add_annealing(batches, step, annealing_scale=1.0):
    annealing = 0.1 + np.exp(-0.1 * max(step - 10, 0)) * 500
