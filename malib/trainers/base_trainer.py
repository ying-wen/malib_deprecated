...
from malib.misc.logger import logger
from garage.sampler.parallel_sampler import singleton_pool
from garage.tf.plotter import Plotter
from malib.samplers import MASampler
...
class Trainer:
    ...
    def setup(self, agents, env, sampler_cls=None, sampler_args=None):
        ...
        self.agents = agents
        self.env = env
        self.policies = [agent.policy for agent in self.agents]

        if sampler_args is None:
            sampler_args = {}

        if sampler_cls is None:
            sampler_cls = MASampler
        self.sampler = sampler_cls(self.env, self.agents, **sampler_args)
        self.has_setup = True
    ...
    def train(self,
        ...
        if batch_size is None:
            if isinstance(self.sampler, MASampler):
