        ...
        for step in range(self.steps):
            ...
            for extra_experience in self.extra_experiences:
                if extra_experience == "annealing":
                    batch = add_annealing(
                        [batch], step - self.exploration_steps, annealing_scale=1.0
                    )[0]
                elif extra_experience == "target_actions":
                    batch = add_target_actions(
                        [batch], [self.agent], self.batch_size, multiagent=False
                    )[0]
                elif extra_experience == "recent_experiences":
                    batch = add_recent_batches([batch], [self.agent], self.batch_size)[
                        0
                    ]
