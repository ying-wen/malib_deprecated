...
class Serializable(object):
    def __initialize(self, locals_):
        ...
        var_positional_keys = [
            p.name for p in signature.parameters.values() if p.kind == p.VAR_POSITIONAL
        ]

        keyword_keys = [
            p.name for p in signature.parameters.values() if p.kind == p.KEYWORD_ONLY
        ]
        ...
        positional_values = [locals_[key] for key in positional_keys if key in locals_]
        var_positional_values = (
            locals_.get(var_positional_keys[0], ()) if var_positional_keys else ()
        )
        keyword_values = {key: locals_[key] for key in keyword_keys if key in locals_}
        var_keyword_values = (
            locals_.get(var_keyword_keys[0], {}) if var_keyword_keys else {}
        )
    ...
    def clone(cls, instance, **kwargs):
        ...
        ), "Can only clone Serializable objects. Got: {}" "".format(type(instance))
