...
class PR2Agent(OffPolicyAgent):
    ...
    @tf.function
    def opponent_policy_loss(
        self, recent_observations, recent_actions, recent_opponent_actions
    ):
        ...
        prior_actions = self._opponent_policy.get_actions(
            [recent_observations, recent_actions]
        )
        loss = tf.reduce_mean(
            tf.losses.mean_squared_error(recent_opponent_actions, prior_actions)
        )
    ...
    @tf.function
    def opponent_svgd_policy_loss(self, observations, actions, annealing):
        n_updated = int(self._value_n_particles * self._kernel_update_ratio)
        n_fixed = self._value_n_particles - n_updated

        @tf.function
        def get_repeated_actions(n):
            repeated_observations = tf.tile(observations[:, None, :], (1, n, 1))
            repeated_observations = tf.reshape(
                repeated_observations, (-1, self._observation_flat_dim)
            )

            repeated_actions = tf.tile(actions[:, None, :], (1, n, 1))
            repeated_actions = tf.reshape(repeated_actions, (-1, self._action_flat_dim))

            repeated_opponent_actions = self._opponent_policy.get_actions(
                [repeated_observations, repeated_actions]
            )

            repeated_opponent_actions = tf.reshape(
                repeated_opponent_actions, (-1, n, self._opponent_action_flat_dim)
            )
        ...
        tf_utils.assert_shape(
            fixed_opponent_actions, [None, n_fixed, self._opponent_action_flat_dim]
        )

        fixed_observations = tf.reshape(
            tf.tile(observations[:, None, :], (1, n_fixed, 1)),
            (-1, self._observation_flat_dim),
        )

        fixed_actions = tf.reshape(
            tf.tile(actions[:, None, :], (1, n_fixed, 1)), (-1, self._action_flat_dim)
        )

        @tf.function
        def get_grad_log_p(opponent_actions):
            opponent_actions = tf.reshape(
                opponent_actions, (-1, self._opponent_action_flat_dim)
            )

            q_values = self._qf.get_values(
                [fixed_observations, fixed_actions, opponent_actions]
            )
            ...
            fixed_opponent_actions = tf.reshape(
                opponent_actions, (-1, n_fixed, self._opponent_action_flat_dim)
            )
        ...
        tf_utils.assert_shape(
            grad_log_p, [None, n_fixed, 1, self._opponent_action_flat_dim]
        )
        ...
        kernel_dict = adaptive_isotropic_gaussian_kernel(
            xs=fixed_opponent_actions, ys=updated_opponent_actions
        )
        ...
        tf_utils.assert_shape(
            action_gradients, [None, n_updated, self._opponent_action_flat_dim]
        )

        gradients = tape.gradient(
            updated_opponent_actions,
    ...
    def critic_loss(
        ...
        next_observations = tf.tile(
            next_observations[:, None, :], (1, self._value_n_particles, 1)
        )
        next_observations = tf.reshape(
            next_observations, (-1, self._observation_flat_dim)
        )
        ...
        critic_loss = critic_loss + self._td_errors_loss_fn(
            reduction=tf.losses.Reduction.NONE
        )(td_targets, ind_q_values)
    ...
    def actor_loss(self, observations, annealing, weights=None):
        ...
        observations = tf.tile(
            observations[:, tf.newaxis, :], (1, self._value_n_particles, 1)
        )
