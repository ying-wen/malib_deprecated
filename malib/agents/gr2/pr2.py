...
class PR2Agent(OffPolicyAgent):
    ...
    def _train(self, batch, weights=None):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            assert opponent_policy_variable, 'No opponent policy variables to optimize.'
            tape.watch(opponent_policy_variable)
            opponent_policy_loss_fn = self.opponent_policy_loss
            if self._loss_type == 'soft':
                opponent_policy_loss_fn = self.soft_opponent_policy_loss
            opponent_policy_loss = opponent_policy_loss_fn(batch['observations'],
                                                             batch['actions'],
                                                             batch['annealing'])
        tf.debugging.check_numerics(opponent_policy_loss, 'opponent policy loss is inf or nan.')
        opponent_policy_grads = tape.gradient(opponent_policy_loss, opponent_policy_variable)
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            assert critic_variables, 'No qf variables to optimize.'
            tape.watch(critic_variables)
            critic_loss_fn = self.critic_loss
            if self._loss_type == 'soft':
                critic_loss_fn = self.soft_critic_loss
            critic_loss = critic_loss_fn(batch['observations'],
                                           batch['actions'],
                                           batch['opponent_actions'],
                                           batch['rewards'],
                                           batch['next_observations'],
    ...
    def soft_opponent_policy_loss(self,
                             observations,
                             actions,
                             annealing
                             ):
        ...
        opponent_policy_loss = annealing - opponent_actions_log_pis + q_values
    ...
    def soft_critic_loss(self,
        ...
        next_opponent_actions = self._opponent_policy.get_actions([next_observations, next_actions])
        next_opponent_actions_log_pis = self._opponent_policy.log_pis([next_observations, next_actions],
                                                                      next_opponent_actions)

        q_value_targets = self._target_qf.get_values([next_observations, next_actions, next_opponent_actions])

        q_value_targets = q_value_targets - annealing * next_opponent_actions_log_pis
    ...
    def critic_loss(self,
        ...
        next_opponent_actions = tf.random.uniform(
                                                    (
                                                        next_observations.shape[0],
                                                        self._opponent_action_flat_dim
                                                    ),
                                                    *(-1., 1.)
        )
        ...
        print('next_opponent_actions', next_opponent_actions.shape)
        q_value_targets = self._target_qf.get_values([next_observations, next_actions, next_opponent_actions])
