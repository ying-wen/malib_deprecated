...
class PR2SoftAgent(OffPolicyAgent):
    ...
    @tf.function
    def prior_loss(self, recent_observations, recent_actions, recent_opponent_actions):
        # actions = self._policy.get_actions(recent_observations)
        log_pis = self._prior.log_pis(
            [recent_observations, recent_actions], recent_opponent_actions
        )
    ...
    def opponent_policy_loss(self, observations, annealing):
        ...
        opponent_policy_loss = tf.reduce_mean(
            opponent_actions_log_pis
            - prior_log_pis
            - q_values
            + annealing * actions_log_pis
        )
