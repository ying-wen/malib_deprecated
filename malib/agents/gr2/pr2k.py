...
class PR2KSoftAgent(OffPolicyAgent):
    ...
    def _train(self, batch, weights=None):
        prior_variable = (
            self._prior_policy.trainable_variables
            + self._opponent_prior_policy.trainable_variables
        )
        ...
        tf_utils.apply_gradients(
            prior_grads, prior_variable, self._prior_optimizer, self._gradient_clipping
        )
    ...
    def prior_loss(
        ...
        log_pis = self._opponent_prior_policy.log_pis(
    ...
    def opponent_policy_loss(self, observations, annealing):
        ...
        prior_log_pis = self._opponent_prior_policy.log_pis(
    ...
    def critic_loss(
        ...
        next_actions_log_pis = self._policy.log_pis(
            [next_observations], k, all_next_actions
        )
        ...
        q_value_targets = (
            q_value_targets
            - annealing * next_actions_log_pis
            - next_opponent_actions_log_pis
        )  # + next_prior_log_pis
    ...
    def actor_loss(self, observations, annealing, weights=None):
        ...
        actor_loss = annealing * tf.reduce_mean(actions_log_pis) - tf.reduce_mean(
            q_values
        )

        inter_improvement_loss = 0.0
        for k_tile in range(k, 1, -2):
            ...
            higher_q = self._qf.get_values(
                [observations, higher_actions, middle_opponent_actions]
            )
            lower_q = self._qf.get_values(
                [observations, lower_actions, middle_opponent_actions]
            )
