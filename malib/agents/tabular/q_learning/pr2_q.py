...
class RRQAgent(QAgent):
    ...
    def update(self, s, a, o, r, s2, env, done=False, tau=0.5):
        ...
        self.count_OS[s][o] += 1.0
        decay_alpha = self.step_decay()
        if self.phi_type == "count":
            count_sum = np.reshape(
                np.repeat(np.sum(self.count_AOS[s], 1), self.action_num),
                (self.action_num, self.action_num),
            )
            self.opponent_best_pi[s] = self.count_AOS[s] / (count_sum + 0.1)
            self.opponent_best_pi[s] = self.opponent_best_pi[s] / (
                np.sum(self.opponent_best_pi[s]) + 0.1
            )
        elif self.phi_type == "norm-exp":
            self.Q_A_reshaped = np.reshape(
                np.repeat(self.Q_A[s], self.action_num),
                (self.action_num, self.action_num),
            )
            ...
            self.opponent_best_pi[s] = self.opponent_best_pi[s] / np.reshape(
                np.repeat(np.sum(self.opponent_best_pi[s], 1), self.action_num),
                (self.action_num, self.action_num),
            )
        ...
        if done:
            ...
            self.Q_A[s][a] = self.Q_A[s][a] + decay_alpha * (
                r + self.gamma * V - self.Q_A[s][a]
            )
        if self.verbose:
            print(self.epoch)
    ...
    def update_policy(self, s, a, game):
        if self.a_policy == "softmax":
            self.pi[s] = softmax(
                np.sum(np.multiply(self.Q[s], self.opponent_best_pi[s]), 1)
            )
        else:
...
class GRRQAgent(QAgent):
    def __init__(
        self, id_, action_num, env, k=0, phi_type="count", a_policy="softmax", **kwargs
    ):
        super().__init__(id_, action_num, env, **kwargs)
        self.name = "GRRQ"
        self.k = k
