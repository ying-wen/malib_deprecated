...
class FullyCentralizedAgent(OffPolicyAgent):
    def __init__(
        self,
        env_specs,
        policy,
        ...
        self.n = env_specs.agent_num
        self.action_dim = env_specs.action_space[0].shape[0]
        self.observation_dim = env_specs.observation_space[0].shape[0]
        ...
        super(FullyCentralizedAgent, self).__init__(
            observation_space,
            action_space,
            policy,
            qf,
    ...
    def act(self, observation, step=None, use_target=False):
        observation = np.reshape(observation, (-1, self.n, self.observation_dim))
        if observation.shape[0] == 1:
            observation = np.reshape(observation, (self.n, self.observation_dim))
        if use_target:
    ...
    @tf.function
    def critic_loss(
        self, observations, actions, rewards, next_observations, weights=None
    ):
        ...
        rewards = tf.reshape(rewards, shape=(-1, self.n))
        observations = tf.reshape(
            observations, shape=(-1, self.n, self.observation_dim)
        )
        next_observations = tf.reshape(
            next_observations, shape=(-1, self.n, self.observation_dim)
        )
        actions = tf.reshape(actions, shape=(-1, self.n, self.action_dim))

        target_actions = self._target_policy.get_actions(next_observations)
        target_actions = tf.reshape(target_actions, shape=(-1, self.n, self.action_dim))
        ...
        target_q_values = tf.reshape(target_q_values, shape=(-1, self.n))
        ...
        q_values = tf.reshape(q_values, shape=(-1, self.n))
