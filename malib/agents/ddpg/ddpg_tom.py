...
class DDPGToMAgent(OffPolicyAgent):
    ...
    def act(self, observation, step=None, use_target=False):
        ...
        opponent_action = self._opponent_policy.get_actions_np(np.array([observation]))[
            0
        ]
        policy_input = [observation, opponent_action]
        ...
        if self._exploration_strategy is not None and self._exploration_status:
            ...
            return self._exploration_strategy.get_action(
                self._train_step, policy_input, self._policy, extend_dim=False
            )
        policy = self._policy
    ...
    def critic_loss(
        ...
        target_opponent_actions = self._opponent_policy.get_actions_np(
            next_observations
        )
        target_actions = self._target_policy.get_actions_np(
            [next_observations, target_opponent_actions]
        )
        target_actions = np.concatenate((target_actions, target_opponent_actions), 1)
        target_critic_input = [
            next_observations.astype(np.float32),
            tf.cast(target_actions, tf.float32),
        ]
        ...
        critic_net_input = [
            observations,
            np.concatenate((actions, opponent_actions), 1),
        ]
    ...
    def opponent_loss(self, recent_observations, recent_opponent_actions):
        predicted_opponent_actions = self._opponent_policy.get_actions(
            recent_observations
        )
        loss = 0.5 * tf.reduce_mean(
            tf.square(predicted_opponent_actions - recent_opponent_actions)
        )
        return loss
