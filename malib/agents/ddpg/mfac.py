...
class MFACAgent(OffPolicyAgent):
    def __init__(
        self,
        env_specs,
        policy,
    ...
    def critic_loss(
        ...
        """
        # opponent_actions = tf.reduce_mean(opponent_actions, axis=-1)
        # print('opponent_actions', opponent_actions)
        opponent_actions = tf.reshape(
            opponent_actions,
            (
                -1,
                self._env_specs.agent_num - 1,
                self._env_specs.action_space[0].shape[0],
            ),
        )
        opponent_actions = tf.math.reduce_mean(opponent_actions, axis=1)
        rewards = tf.reshape(rewards, shape=(-1, 1))
        target_actions = tf.reshape(
            target_actions,
            (-1, self._env_specs.agent_num, self._env_specs.action_space[0].shape[0]),
        )

        target_agent_actions = target_actions[:, 0]
        target_opponent_actions = target_actions[:, 1:]
        target_opponent_actions = tf.reduce_mean(target_opponent_actions, axis=1)
        target_actions = tf.concat(
            (target_agent_actions, target_opponent_actions), axis=1
        )
