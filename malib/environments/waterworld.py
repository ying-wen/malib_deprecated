...
class MAWaterWorld_mod(BaseGame):
    ...
    def step(self, action_Np2):
        ...
        for npu, pursuer in enumerate(self._pursuers):
            distfromobst_No = ssd.cdist(
                np.expand_dims(pursuer.position, 0), self.obstaclesx_No_2
            )
        ...
        for nev, evader in enumerate(self._evaders):
            distfromobst_No = ssd.cdist(
                np.expand_dims(evader.position, 0), self.obstaclesx_No_2
            )
        ...
        for npo, poison in enumerate(self._poisons):
            distfromobst_No = ssd.cdist(
                np.expand_dims(poison.position, 0), self.obstaclesx_No_2
            )
        ...
        sensed_obdistfeatures_Np_K[sensedmask_ob_Np_K] = closest_ob_dist_Np_K[
            sensedmask_ob_Np_K
        ]
        # Evaders
        ...
        sensed_evdistfeatures_Np_K[sensedmask_ev_Np_K] = closest_ev_dist_Np_K[
            sensedmask_ev_Np_K
        ]
        # Poison
        ...
        sensed_podistfeatures_Np_K[sensedmask_po_Np_K] = closest_po_dist_Np_K[
            sensedmask_po_Np_K
        ]
        # Allies
        ...
        sensed_pudistfeatures_Np_K[sensedmask_pu_Np_K] = closest_pu_dist_Np_K[
            sensedmask_pu_Np_K
        ]
        ...
        sensed_evspeedfeatures_Np_K = self._extract_speed_features(
            evadersv_Ne_2, closest_ev_idx_Np_K, sensedmask_ev_Np_K
        )
        # Poison
        sensed_pospeedfeatures_Np_K = self._extract_speed_features(
            poisonv_Npo_2, closest_po_idx_Np_K, sensedmask_po_Np_K
        )
        # Allies
        sensed_puspeedfeatures_Np_K = self._extract_speed_features(
            pursuersv_Np_2, closest_pu_idx_Np_K, sensedmask_pu_Np_K
        )
        ...
        if ev_caught.size:
            for evcaught in ev_caught:
                ...
                self._evaders[evcaught].set_position(
                    self._respawn(
                        self._evaders[evcaught].position,
                        self._evaders[evcaught]._radius,
                    )
                )
        ...
        if po_caught.size:
            for pocaught in po_caught:
                ...
                self._poisons[pocaught].set_position(
                    self._respawn(
                        self._poisons[pocaught].position,
                        self._poisons[pocaught]._radius,
                    )
                )
        ...
        if self.reward_mech == "global":
            rewards += (
                (len(ev_caught) * self.food_reward)
                + (len(po_caught) * self.poison_reward)
        ...
        if self._speed_features:
            sensorfeatures_Np_K_O = np.c_[
                sensed_obdistfeatures_Np_K,
                sensed_evdistfeatures_Np_K,
                sensed_evspeedfeatures_Np_K,
                sensed_podistfeatures_Np_K,
                sensed_pospeedfeatures_Np_K,
                sensed_pudistfeatures_Np_K,
        ...
        assert all(
            [
                obs.shape == agent.observation_space.shape
    ...
    def render(self, screen_size=800, rate=10, mode="human"):
        ...
        for iobs, obstaclex_2 in enumerate(self.obstaclesx_No_2):
            ...
            cv2.circle(
                ...
                int(self.obstacle_radius * screen_size),
                color,
                -1,
                ...
                cv2.line(
                    ...
                    tuple(
                        (
                            (
                                pursuer.position
                                + pursuer._sensor_range * pursuer.sensors[k]
                            )
                            * screen_size
                        ).astype(int)
                    ),
...
if __name__ == "__main__":
    ...
    obs = env.reset()
    while True:
        obs, rew, _, _ = env.step(env.np_random.randn(10) * 0.5)
        if rew.sum() > 0:
            print(rew)
        env.render()
