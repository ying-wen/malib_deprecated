...
def fwd_gradients(
    ys, xs, grad_xs=None, stop_gradients=None, colocate_gradients_with_ops=True
):
...
class SymplecticOptimizer(tf.train.Optimizer):
    ...
    def compute_gradients(
        ...
        return self._gd.compute_gradients(
            loss,
            var_list,
            gate_gradients,
            aggregation_method,
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        at_v = list_divide_scalar(list_subtract(ht_v, h_v), 2.0)
        if self._use_signs:
